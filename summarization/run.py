...
if __name__ == "__name__":
    # Initialize the TIRA Client, which allows interaction with the TIRA platform
    tira_client = Client()

    # Load the dataset using the TIRA Client

    # The dataframe's index is set to the 'id' column for easy reference
    dataset = tira_client.pd.inputs(
    ...
    if 'text' in dataset.columns:
        ...
        dataset["summary"] = dataset["text"].str.split("\n").str[:2].str.join("\n")
        
        # Drop the original 'text' column from the dataframe as it's no longer needed
        dataset = dataset.drop(columns=["text"]).reset_index()
